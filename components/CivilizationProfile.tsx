...
import type { CivilizationProfile, TimelineEvent } from '@/lib/civilizations';
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="label" style={{ color: health >= 80 ? '#00B4A8' : health >= 60 ? '#D8B36A' : '#A33A4A' }}>{health}%</span>
      </div>
      <div className="h-2 w-full bg-rule/20">
        <div className="h-full transition-all duration-1000 ease-out" style={{ width: `${health}%`, background: `linear-gradient(90deg, ${color}, ${getColor()})` }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative h-2 w-full bg-rule/20">
        ...
        <div className="absolute right-0 top-1/2 h-3 w-px -translate-y-1/2 bg-gold" />
      </div>
...
interface Props {
  ...
  otherCivs: { slug: string; name: string; genre: string; color: string }[];
}

export function CivilizationProfileClient({ profile, otherCivs }: Props) {
  ...
  return (
    <main className="bg-void text-ivory min-h-screen">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`label whitespace-nowrap px-6 py-4 transition-all duration-300 ${
                  activeTab === tab.id ? 'border-b-2 text-ivory' : 'text-ivory/40 hover:text-ivory/70'
                }`}
          ...
          <div className="space-y-16">
            <Reveal>
              <div className="max-w-3xl">
                ...
                <p className="mt-6 text-lg leading-relaxed text-ivory/80" style={{ lineHeight: '1.8' }}>{profile.description}</p>
              </div>
            </Reveal>
            <Reveal>
              ...
              <Reveal>
                <div className="border border-rule bg-void-deep p-6">
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between"><span className="label text-ivory/40">Location</span><span className="text-sm text-ivory/80">{profile.location}</span></div>
                    <div className="flex items-center justify-between"><span className="label text-ivory/40">Treasury</span><span className="font-mono text-sm text-gold">${profile.governance.treasury.toLocaleString()}</span></div>
                    <div className="flex items-center justify-between"><span className="label text-ivory/40">Leader</span><span className="text-sm text-ivory/80">{profile.governance.currentLeader}</span></div>
                  </div>
                </div>
              </Reveal>
              <Reveal>
                  ...
                  {profile.activeQuests.map((quest) => (
                    <div key={quest.title} className="border border-rule bg-void-deep p-6 transition-all duration-500 hover:border-gold/20">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="label" style={{ color: QUEST_STATUS[quest.status].color }}>{QUEST_STATUS[quest.status].label}</span>
                      </div>
              ...
              {profile.timeline.map((event, i) => (
                <Reveal key={event.year} delay={i * 0.08}>
                  <div className="relative mb-12 pl-16">
                    <div className="absolute left-3.5 top-1 flex h-5 w-5 items-center justify-center rounded-full border text-[10px]" style={{ borderColor: profile.color, backgroundColor: `${profile.color}15` }}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {profile.agents.map((agentName) => {
                    ...
                    return (
                      <Link key={agentName} href={`/agents/${agentName.toLowerCase()}`} className="group flex items-center gap-4 border border-rule bg-void-deep p-6 transition-all duration-500 hover:border-gold/20">
                        <div className="flex h-14 w-14 items-center justify-center border" style={{ borderColor: `${color}30`, backgroundColor: `${color}10` }}>
                  ...
                  {profile.keyFigures.map((figure) => (
                    <div key={figure.name} className="border border-rule bg-void-deep p-6 transition-all duration-500 hover:border-gold/20">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="label border px-2 py-0.5" style={{ color: STATUS_COLORS[figure.status], borderColor: `${STATUS_COLORS[figure.status]}30` }}>{figure.status}</span>
                      </div>
            ...
            <Reveal>
              <div>
                ...
                <div className="mt-6 border border-rule bg-void-deep p-8">
                  <p className="text-lg leading-relaxed text-ivory/80 italic" style={{ fontFamily: 'var(--font-display), serif', lineHeight: '1.8' }}>&ldquo;{profile.governance.constitution}&rdquo;</p>
                </div>
              </div>
            </Reveal>
            <Reveal>
              <div className="border border-rule bg-void-deep p-8">
                ...
                <h3 className="mt-3 font-display text-2xl text-ivory" style={{ fontFamily: 'var(--font-display), serif' }}>{profile.governance.currentLeader}</h3>
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div><span className="label text-ivory/40">Treasury</span><p className="mt-1 font-mono text-lg text-gold">${profile.governance.treasury.toLocaleString()}</p></div>
                  <div><span className="label text-ivory/40">Active Proposals</span><p className="mt-1 font-mono text-lg text-ivory">{profile.governance.proposals.length}</p></div>
                </div>
              </div>
            </Reveal>
            <Reveal>
              <div>
                <div className="mb-8 flex items-baseline gap-6"><span className="label" style={{ color: profile.color }}>Proposals</span><span className="h-px flex-1 bg-rule" /></div>
                  ...
                  {profile.governance.proposals.map((proposal) => (
                    <div key={proposal.title} className="border border-rule bg-void-deep p-6 transition-all duration-500 hover:border-gold/20">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="label" style={{ color: PROPOSAL_STATUS[proposal.status].color }}>{PROPOSAL_STATUS[proposal.status].label}</span>
                      </div>
                      ...
                      {(proposal.status === 'voting' || proposal.status === 'passed' || proposal.status === 'rejected') && (
                        <div className="mt-4"><VoteBar forVotes={proposal.votesFor} againstVotes={proposal.votesAgainst} quorum={proposal.quorum} /></div>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-16 max-w-3xl">
            <Reveal>
              <div>
                ...
                <div className="mt-6 border border-rule bg-void-deep p-8">
                  <p className="font-display text-lg leading-relaxed text-ivory/85 italic" style={{ fontFamily: 'var(--font-display), serif', lineHeight: '1.8' }}>&ldquo;{profile.lore.creationMyth}&rdquo;</p>
                </div>
              </div>
            </Reveal>
            <Reveal>
              <div>
                ...
                <div className="mt-6 border border-rule bg-void-deep p-8">
                  <p className="text-lg leading-relaxed text-ivory/80" style={{ lineHeight: '1.8' }}>{profile.lore.coreBelief}</p>
                </div>
              </div>
            </Reveal>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <Reveal>
                <div className="border border-rule bg-void-deep p-8">
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-ivory/70" style={{ lineHeight: '1.7' }}>{profile.lore.taboo}</p>
                </div>
              </Reveal>
            </div>
          </div>
      ...
      <section className="border-t border-rule bg-void-deep">
        <div className="mx-auto max-w-[1440px] px-6 py-16 md:px-10 lg:px-16">
          <Reveal>
            <div className="mb-8 text-center"><span className="label text-gold">Explore Other Worlds</span></div>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
              {otherCivs.map((civ) => (
                <Link key={civ.slug} href={`/civilization/${civ.slug}`} className="group border border-rule bg-void p-4 text-center transition-all duration-500 hover:border-gold/30">
